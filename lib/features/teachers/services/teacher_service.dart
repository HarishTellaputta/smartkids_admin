...
class TeacherService {
  ...
  Future<List<Teacher>> getTeachersBySchool(
    int schoolId,
  ) async {
    try {
      final response = await _dio.get(
      ...
      throw Exception(
        'Failed to load school teachers: $e',
      );
    }
  }
  ...
  Future<List<Teacher>> getTeachersByStatus(
    String status,
  ) async {
    try {
      final response = await _dio.get(
        '/api/v1/teachers/status/$status',
      );
      ...
      throw Exception(
        'Failed to load teachers by status: $e',
      );
    }
  }
